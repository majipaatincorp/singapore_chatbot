...
# ------------------ Loading Embedding Model and DB ------------------

app_logger.info("Loading HuggingFace embeddings...")
try:
    model = SentenceTransformer("sentence-transformers/all-MiniLM-L6-v2")
    app_logger.info("Embeddings loaded successfully")
except Exception as e:
    app_logger.error(f"Failed to load HuggingFace embeddings: {e}")
    raise RuntimeError(f"Failed to initialize embeddings model: {e}")

app_logger.info("Loading vector database...")
try:
    search_client = SearchClient(endpoint=os.environ.get("endpoint"), index_name=os.environ.get("index_name"), credential=AzureKeyCredential(os.environ.get("key")))
    app_logger.info("Vector database connected successfully")

except Exception as e:
    app_logger.error(f"Failed to connect vector database: {e}")
